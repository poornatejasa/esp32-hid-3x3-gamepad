...
static const uint8_t hid_report_reference[] ={
    0x01,   // Report ID
    0x01    // Input Report
};

static const uint8_t hid_output_report_reference[] ={
    0x01,   // Report ID
    0x02    // Output Report
};
    ...
    if (ctxt->op == BLE_GATT_ACCESS_OP_READ_DSC) {
        if (ble_uuid_u16(ctxt->dsc->uuid) == REPORT_REFERENCE_UUID) {
            if (attr_handle == hid_output_report_handle + 1) {
                os_mbuf_append(ctxt->om, hid_output_report_reference,
                               sizeof(hid_output_report_reference));
                ESP_LOGI(TAG, "Output Report Reference Read");
            } else {
                os_mbuf_append(ctxt->om, hid_report_reference,
                               sizeof(hid_report_reference));
            ...
            switch (ble_uuid_u16(ctxt->chr->uuid)){
                ...
                case HID_REPORT_UUID:
                    if (attr_handle == hid_output_report_handle) {
                        os_mbuf_append(ctxt->om, &keyboard_led_state,
                                       sizeof(keyboard_led_state));
        ...
        case BLE_GATT_ACCESS_OP_WRITE_CHR:
            ...
            switch (ble_uuid_u16(ctxt->chr->uuid)){
                case HID_PROTOCOL_MODE_UUID: {
                    ...
                    hid_protocol_mode = requested_mode;
                    ESP_LOGI(TAG, "Protocol Mode = %d", hid_protocol_mode);
                    return 0;
                }
                ...
                case HID_REPORT_UUID:
                case BOOT_KEYBOARD_OUTPUT_UUID:
                    keyboard_led_state = ctxt->om->om_data[0];
                    ESP_LOGI(TAG, "Keyboard LED state = 0x%02X",
                             keyboard_led_state);
                    return 0;
                default:
                    return BLE_ATT_ERR_UNLIKELY;
...
esp_err_t ble_transport_send_report(const uint8_t *report, size_t len){ 
    ...
    if (!notifications_enabled) {
        ESP_LOGW(TAG, "%s-protocol notifications are not enabled",
                 hid_protocol_mode == 0 ? "Boot" : "Report");
        return ESP_ERR_INVALID_STATE;
    }
